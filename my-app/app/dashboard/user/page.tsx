...
export default async function UserDashboard() {
  ...
  const activeClaimsList = claimsRaw.filter(c => c.status === "PENDING" || c.status === "APPROVED");
  const pastClaimsList = claimsRaw.filter(c => c.status === "REJECTED" || c.status === "CLOSED");
  ...
  const approvedTotal = claimsRaw
    ...
    .reduce((sum, claim) => {
        const cost = claim.motor_claim?.estimated_repair_cost || claim.burglary_claim?.estimated_repair_cost || 0;
    ...
    <div className="flex-1 space-y-6 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Est. value of approved claims</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Claims currently being processed or requiring attention</CardDescription>
        </CardHeader>
        <CardContent>
